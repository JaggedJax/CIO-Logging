 ...
 *
 * Usage:
 * $log = new CIOLogger('/var/log/', CIOLogger::INFO);
...
class CIOLogger {
	 ...
	 */
	const NO_ARGUMENTS = 'CIOLogger::NO_ARGUMENTS';
	...
	/**
	 * Prefix to attach to log ID. Use to be able to differentiate what program a log ID is from.
	 * @var string 
	 */
	private $_logIdPrefix = '';
	 ...
	 * @var integer
	 */
	private static $_defaultPermissions = 0666;

	/**
	 * Array of CIOLogger instances, part of Singleton pattern
	 * @var array
	 */
	 ...
	 * @return CIOLogger
	 */
	public static function instance($logDirectory = false, $severity = false, $prefix='') {
		if ($severity === false) {
		...
		self::$instances[$logDirectory] = new self($logDirectory, $severity, $prefix);
	 ...
	 *
	 * @param string  $logDirectory File path to the logging directory
	 * @param integer $severity Minimum severity level to log
	 * @param string $prefix Prefix log IDs with this string. Use to be able to differentiate what program a log ID is from.
	 * @return void
	 */
	public function __construct($logDirectory, $severity, $prefix='') {
		require dirname(__FILE__) . './CIOHelperFunctions.php';
		require dirname(__FILE__) . './CIOCrypt.php';
		$logDirectory = rtrim($logDirectory, '\\/');
		$this->_canFork = function_exists('pcntl_fork');
		$this->_canLock = $this->_canFork && function_exists('flock');
		$this->_logIdPrefix = $prefix;
		...
		$this->_logFilePath = $logDirectory
				. DIRECTORY_SEPARATOR
				.($this->_logIdPrefix ? $this->_logIdPrefix.'-' : '')
				. 'log_'
				. date('Y-m-d')
				. '.txt';
	...
	/**
	 * Sets the date format used by all instances of CIOLogger
	 * 
	...
	public function logInfo($line, $args = self::NO_ARGUMENTS) {
		return $this->log($line, self::INFO, $args);
	}
	...
	public function logNotice($line, $args = self::NO_ARGUMENTS) {
		return $this->log($line, self::NOTICE, $args);
	}
	...
	public function logWarn($line, $args = self::NO_ARGUMENTS) {
		return $this->log($line, self::WARN, $args);
	}
	...
	public function logError($line, $args = self::NO_ARGUMENTS) {
		return $this->log($line, self::ERR, $args);
	}
	...
	public function logFatal($line, $args = self::NO_ARGUMENTS) {
		return $this->log($line, self::FATAL, $args);
	}
	...
	public function logAlert($line, $args = self::NO_ARGUMENTS) {
		return $this->log($line, self::ALERT, $args);
	}
	...
	public function logCrit($line, $args = self::NO_ARGUMENTS) {
		return $this->log($line, self::CRIT, $args);
	}
	...
	public function logEmerg($line, $args = self::NO_ARGUMENTS) {
		return $this->log($line, self::EMERG, $args);
	}

	/**
	 * Writes a $line to the log with the given severity.
	 * Attempts to fork a child process and return immediately.
	 *
	 * @param string  $line	Text to add to the log
	 * @param integer $severity	Severity level of log message (use constants)
	 * @return string The Log ID. Returns false if not logged.
	 */
	public function log($line, $severity, $args = self::NO_ARGUMENTS) {
		if ($this->_severityThreshold >= $severity) {
			$status = $this->_getTimeLine($severity);
			
			$logID = date('ymdHis').'-'.CIOCrypt::rand_alphanumeric(3);
			if ($this->_logIdPrefix){
				$logID = $this->_logIdPrefix.'-'.$logID;
			}
			$server = ($_SERVER['REMOTE_ADDR']) ? ' ['.$_SERVER['REMOTE_ADDR'].'] '
					: ' ';

			$line = $logID.$server.$status.' '.$line;
			...
			if ($this->_canFork){
				$pid = pcntl_fork();
				/*
				 * Return immediately if we are the parent
				 * Continue if there was a forking error and pid is -1
				 */
				if ($pid && $pid != -1) {
					 return $logID;
				}
			...
			// exit if we're the child thread
			if($this->_canFork && $pid === 0){
				exit(0);
			}
			return $logID;
		}
	 ...
	 * Attempts to lock file while writing if possible.
	 *
	...
	public function writeFreeFormLine($line) {
		if ($this->_logStatus == self::STATUS_LOG_OPEN && $this->_severityThreshold != self::OFF) {
			$haveLock = (!$this->_canLock || flock($this->_fileHandle, LOCK_EX)) ? true : false;
			if ($haveLock && fwrite($this->_fileHandle, $line) === false) {
				$this->_messageQueue[] = $this->_messages['writefail'];
			}
			else if (!$haveLock){
				$this->_messageQueue[] = $this->_messages['lockfail'];
			}
			if($haveLock && $this->_canLock){
				// Unlock
	...
	private function _getTimeLine($level) {
		// Time removed because the Log ID is also the time
